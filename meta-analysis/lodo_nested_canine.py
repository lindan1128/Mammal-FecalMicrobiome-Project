...
# Date: 2026-03-25
"""
LODO nested CV for canine IBD.

Same structure as bovine version - each fold holds out one study,
meta-analysis on training studies selects signature genera (via lodo_meta_step1.R).
Note: with 3 studies, each training fold has only 2 studies (k>=2 required).

outputs go to canine/:
...
BASE_DIR = "/Users/lindan/Dropbox/PhD/Projects/Animal_Microbiome_cb/metadata/canine"
OUT_DIR  = os.path.join(SCRIPT_DIR, "canine")
...
DIR_THRESH = 0.5
MIN_K = 2
MIN_STUDY_PRESENCE = 2  # canine has only 3 studies
MODEL_CONFIGS = {
    "RF": {
...
def select_signature_nested_R(train_studies, common_genera):
    ...
    try:
        ...
        # Read signature genera and full meta results
...
def prepare_Xy(study_data, study_ids, genera):
    ...
    Applies per-study CZM + CLR (matching meta-analysis pipeline)."""
    Xs, ys = [], []
...
def run_lodo():
    ...
    min_stable = int(np.ceil(len(STUDIES) * 2 / 3))
    genus_count = {}
    ...
    if importance_rows:
        ...
        print(f"\nFeature importance saved: {len(imp_df)} rows")
...
if __name__ == "__main__":
    ...
    print(f"LODO canine | output: {OUT_DIR}")
