...
def nn_cv_pretrain(data, labels, pretrain_data=None, pretrain_labels=None, n_folds=5, n_repeats=5, strategy="transfer"):
    ...
    for rep in range(n_repeats):
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(balanced_data, balanced_labels)):
            ...
            if pretrain_data is not None and pretrain_labels is not None:
                # With pre-training data: use different strategies based on strategy parameter
                # Combine pre-training data and training data for standardization
                combined_train_data = pd.concat([train_train_data, pretrain_data], ignore_index=True)
                ...
                # Step 1: Pre-training encoder (combined reconstruction and classification loss)
                print(f"      Step 1: Pre-training encoder on {len(pretrain_data)} samples...")
                ...
                model = pretrain_encoder(model, X_pretrain, y_pretrain, epochs=30, 
                                       reconstruction_weight=0.5, classification_weight=0.5)
...
def run_transfer_learning_analysis(datasets, feature_type="full"):
    ...
    for target_id in [1, 2, 3]:
        ...
        for source_id in [1, 2, 3]:
            if source_id != target_id:  # Source and target must be different
                source_data = feature_datasets[source_id]['otu_data']
                source_labels = feature_datasets[source_id]['labels']
                
                # Transfer learning strategy
                print(f"Transfer learning from dataset {source_id} to dataset {target_id}:")
                ...
                # Cross strategy (pre-train on source, evaluate on target without fine-tuning)
                print(f"Cross-dataset evaluation from dataset {source_id} to dataset {target_id}:")
...
def main():
    ...
    for data_type in data_types:
        ...
        full_results = run_transfer_learning_analysis(aligned_datasets, "full")
        ...
        signature_results = run_transfer_learning_analysis(aligned_datasets, "signature")
        # Add data_type to each result
    ...
    # Save results
    output_file = "/Users/lindan/Dropbox/PhD/Projects/Animal_Microbiome/transfer_single_cow.csv"
