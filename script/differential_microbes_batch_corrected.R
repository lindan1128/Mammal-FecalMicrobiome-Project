...
# Output: Differential microbes after MMUPHin and limma batch correction
...
# Function to perform limma batch correction
perform_limma_correction <- function(rel_abundance, meta_data) {
  cat("Performing limma batch correction...\n")
  
  # Transpose data for limma (samples as rows, features as columns)
  otu_relative_t <- t(rel_abundance)
  ...
  design <- model.matrix(~ group, data = meta_data_filtered)
  
  # Prepare covariates
  covariates_list <- list()
  if (!all(is.na(meta_data_filtered$Age))) {
    covariates_list$Age <- meta_data_filtered$Age
  }
  if (!all(is.na(meta_data_filtered$Sex))) {
    covariates_list$Sex <- meta_data_filtered$Sex
  }
  
  # Perform limma batch correction
  tryCatch({
    if (length(covariates_list) > 0) {
      otu_relative_t_lim <- removeBatchEffect(otu_relative_t, 
    ...
    # Transpose back to original format (features as rows, samples as columns)
    otu_relative_t_lim <- t(otu_relative_t_lim)
    
    cat("  Limma correction completed successfully\n")
    return(otu_relative_t_lim)
    
  }, error = function(e) {
    cat("  Error in limma correction:", e$message, "\n")
    cat("  Returning original data\n")
    return(rel_abundance)
  })
}
...
main <- function(otu_files, meta_files, dataset_names, output_dir = ".") {
  ...
  # Limma correction
  limma_corrected <- perform_limma_correction(filtered_rel_abundance, merged_data$meta_data)
  ...
  # Limma corrected
  limma_results <- identify_differential_microbes(limma_corrected, merged_data$meta_data, "limma")
  ...
  limma_summary <- create_summary_stats(limma_results, "limma")
  ...
  limma_plots <- create_visualizations(limma_results, output_dir, "limma")
  
  # Combine all results
  all_results <- rbind(original_results, mmuphin_results, limma_results)
  all_summaries <- rbind(
    data.frame(original_summary, stringsAsFactors = FALSE),
    data.frame(mmuphin_summary, stringsAsFactors = FALSE),
    data.frame(limma_summary, stringsAsFactors = FALSE)
  )
  ...
  write.csv(limma_results, file.path(output_dir, "differential_microbes_limma.csv"), row.names = FALSE)
